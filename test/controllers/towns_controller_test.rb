...
class TownsControllerTest < ActionController::TestCase
 setup do
   @town = towns(:one)
   @inexistingTown = towns(:inexisting)
 end
 ...
 test "should fail if town does not exist" do
   VCR.use_cassette("inexistingTown_ok") do
     assert_no_difference('Town.count') do
       post :create, town: { name: @inexistingTown.name }
     end
   end
 ...
 test "should show town" do
   VCR.use_cassette("showTown_ok") do
     get :show, id: @town
     assert_response :success
   end
 end
 ...
 test "should update town" do
   VCR.use_cassette("updateTown_ok") do
